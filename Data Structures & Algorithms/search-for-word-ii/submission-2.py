class Solution:
    ...
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        for i in range(len(board)):
            for j in range(len(board[i])):
                ...
                self.searchBoard(board, used, words, i, j, self.root)        
        return self.found
    ...
    def searchBoard(self, board, used, words, row, col, root):
        cur = root
        ...
        for neighbor in neighbors:
            if board[neighbor[0]][neighbor[1]] in cur.children:
                self.searchBoard(board, used, words, neighbor[0], neighbor[1], cur)
